...
static void summary(unsigned long long numb, int numthreads)
{
	...
	*msg = '\0';
	if ((Log != STDERR_FILENO) && (StatusLog != 0))
		if (-1 == write(Log,buf,msg-buf)) {}
	if ((Status != 0) && (Quiet == 0))
		if (-1 == write(STDERR_FILENO,buf,msg-buf)) {}
}
...
static int requestOutputVolume(int out, const char *outfile)
{
	...
	if (outfile == Stdout) {
		...
		assert(0 == err);
		char c = 0, msg[] = "\nfilesystem full - please provide enough space and press return when ready...\n";
		if (-1 == write(STDERR_FILENO,msg,sizeof(msg))) {
			errormsg("error accessing controlling terminal for manual filesystem cleanup request: %s\n",strerror(errno));
			return -1;
		}
		do {
	...
	do {
		...
		if (Autoloader) {
			...
			char str[16];
			++Vol;
			snprintf(str,sizeof(str),"%u",Vol);
			setenv("MBUFFER_VOL",str,1);
			snprintf(str,sizeof(str),"%llu",Numin*Blocksize);
			setenv("MBUFFER_NUMIN",str,1);
			snprintf(str,sizeof(str),"%llu",Numout*Blocksize);
			setenv("MBUFFER_NUMOUT",str,1);
			infomsg("requesting new output volume %u with command '%s'\n",Vol,cmd);
			err = system(cmd);
			if (0 < err) {
...
static void *outputThread(void *arg)
{
	...
	for (;;) {
		...
		if ((StartWrite > 0) && (StartRead < 1) && (StartRead < StartWrite)) {
			...
			/* If we're paused, wait for high watermark OR end of input */
			if (watermark_paused) {
				/* Check if input has finished - if so, resume to flush remaining data */
				if (Finish != -1) {
					watermark_paused = 0;
					debugmsg("outputThread: input finished, resuming to flush remaining %.1f%% buffer\n", fill_percent * 100);
				} else {
					static int pause_cycles = 0;
					pause_cycles++;
					
					/* Safety check: if we've been paused for too long with no progress, resume anyway */
					if (pause_cycles > 6000) { /* 60 seconds at 10ms intervals */
						watermark_paused = 0;
						debugmsg("outputThread: pause timeout after 60s, resuming at %.1f%%\n", fill_percent * 100);
						pause_cycles = 0;
					} else {
	...
	while (d) {
		...
		if (d->result) {
			...
			if (d->arg) {
				...
				if (-1 == write(STDERR_FILENO,d->result,strlen(d->result))) {}
				if (Log != STDERR_FILENO)
					if (-1 == write(Log,d->result,strlen(d->result))) {}
			}
		}
		free(d);
		d = n;
...
static void initDefaults()
{
	...
	if (AvP && PgSz) {
		...
		PgSz = 4096; // assumed for calculation
	}
	...
	if (AvP) {
		...
		debugmsg("default Numblocks = %lu, default Blocksize = %llu, buffer size %llukiB\n",Numblocks,Blocksize,(Numblocks*Blocksize)>>10);
	}
	Outsize = Blocksize;
...
int main(int argc, const char **argv)
{
	...
	memcpy(Prefix,progname,PrefixLen);
	Prefix[PrefixLen++] = ':';
	Prefix[PrefixLen++] = ' ';
	...
	if ((read(STDERR_FILENO,&c,1) != -1) || (errno == EAGAIN)) {
		...
		if (-1 == tty) {
			...
			if (-1 == err) {
				warningmsg("failed to redirect stderr to terminal\n");
				Terminal = 0;
			} else {
				Terminal = 1;
			}
